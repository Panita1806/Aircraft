...
st.caption(
    "Aircraft usage • Weather impact • Maintenance risk • Pilot scheduling"
)

# =========================
# LOAD DATA
# =========================
DEFAULT_FILE = "aircraft_training_logs_with_pilots.csv"
...
if "flight_date" in df.columns:
    ...
    start, end = st.sidebar.date_input(
        "Flight date range", (min_d, max_d)
    )
    df = df[(df["flight_date"].dt.date >= start) &
            (df["flight_date"].dt.date <= end)]

# Aircraft
aircrafts = sorted(df["aircraft_id"].unique())
selected_aircraft = st.sidebar.multiselect(
    "Aircraft", aircrafts, default=aircrafts
)
df = df[df["aircraft_id"].isin(selected_aircraft)]

# Pilot
pilots = sorted(df["pilot_id"].unique())
selected_pilots = st.sidebar.multiselect(
    "Pilot", pilots, default=pilots
)
...
selected_weather = st.sidebar.multiselect(
    "Weather condition", weather, default=weather
)
...
k2.metric("Total Flight Hours", round(df["flight_hours"].sum(), 2))
k3.metric("Delayed Flights", int((df["flight_status"] == "Delayed").sum()))
k4.metric("No-Fly Flights", int((df["flight_status"] == "No-Fly").sum()))
...
with c2:
    ...
    plt.bar(
        usage_aircraft["aircraft_id"],
        usage_aircraft["total_flight_hours"]
    )
...
with c2:
    fig = plt.figure(figsize=(6,4))
    plt.bar(
        usage_time["time_of_day"],
        usage_time["total_flight_hours"]
    )
...
risk_df["risk_score"] = (
    0.5 * minmax(risk_df["total_flight_hours"]) +
    0.3 * minmax(risk_df["maintenance_events"]) +
    0.2 * minmax(risk_df["avg_downtime_days"])
).round(2)
...
with c1:
    st.dataframe(
        schedule_df[
            ["flight_date","pilot_id","aircraft_id",
             "risk_score","weather_condition",
             "flight_status","recommendation"]
        ].head(50),
